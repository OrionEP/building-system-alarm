 ...
 * @ass1
 */
public class Building extends Object implements FireDrill, Encodable {
     ...
     *                                  width &lt; Floor.getMinWidth(), or
     *                                  length &lt; Floor.getMinLength()
     ...
     *                                  the floor below is not big enough to
    ...
    /**
     * change the length and witdth to renovate the given floor.
     *
     * @param floorNumber which floor to be renovated.
     * @param newWidth    the new width of the floor.
     * @param newLength   the new length of the floor.
     * @throws IllegalArgumentException when the number or the width or
     *                                  the length is not legal.
     * @throws FloorTooSmallException   the floor is too small to support
     *                                  increased dimension or decreased
     *                                  dimension.
     */
    public void renovateFloor(int floorNumber,
        ...
        if(getFloorByNumber(floorNumber)==null||newLength<Floor.getMinLength()||newWidth<Floor.getMinWidth())
            throw new IllegalArgumentException();
        ...
        if (floorBelow.calculateArea() < newLength * newWidth
                || floorAbove.calculateArea() > newLength * newWidth
                || newLength * newWidth < floor.occupiedArea()) {
    ...
    /**
     * whether this building is equal to the other given building.
     *
     * @param obj other object to compare equality/
     * @return boolean true if equal, other false.
     */
    public boolean equals(Object obj) {
        ...
        return this.name.equals(building.name)
                && getFloors().size() == building.getFloors().size()
                && floors.containsAll(building.floors)
                && building.floors.containsAll(floors);

    }

    /**
     * return the hashcode of the building.
     * <p> two equal building their hashcode are same.
     *
     * @return int the hashcode of the building.
     */
    public int hashCode() {
        int sum = 0;
        ...
        return name.hashCode() + sum + floors.size();
    }
    ...
    /**
     * the machine-readable string representation of this building.
     *
     * @return string encoded string representation of this building.
     */
    @Override
    public String encode() {
        StringBuilder head = new StringBuilder();
        head.append(name).append(System.lineSeparator())
                .append(getFloors().size()).append(System.lineSeparator());
        StringBuilder body = new StringBuilder();
        for (Floor floor : floors) {
            body.append(floor.encode()).append(System.lineSeparator());
        }
