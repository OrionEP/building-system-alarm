...
public class CarbonDioxideSensor extends TimedSensor implements
    ...
    public int getComfortLevel(){
        float difference = (float)Math.abs(getIdealValue()-getCurrentReading());
        if(difference>=getVariationLimit()){
            return 0;
        }else{
            float comfortLevel=(1-(difference/getVariationLimit()))*100;
            return Math.round(comfortLevel);
        }
    }
    /**
     * whether two carbon dioxide sensors are equal.
    ...
    public boolean equals(Object obj){
        ...
        if(sensor.getIdealValue()!=getIdealValue()
                ||sensor.getVariationLimit()!=getVariationLimit()){
            return false;
        }
    ...
    @Override
    public int hashCode(){
        return Objects.hash(idealValue,variationLimit,super.hashCode());
    }
    /**
    ...
    public String encode(){
        return String.format("CarbonDioxideSensor:%s:%d:%d:%d",
                super.encode(),getUpdateFrequency(),idealValue,variationLimit);
